...
- (void)buildView {
    ...
    self.weatherIconView = [[WeatherIconView alloc] init];//实时天气
    [self.tableView addSubview:self.weatherIconView];
    
    self.temperatureView = [[TemperatureView alloc] init];//实时温度
    [self.tableView addSubview:self.temperatureView];
    
    self.humidityView = [[HumidityView alloc] init];//湿度
    [self.tableView addSubview:self.humidityView];
    
    self.sunInfoView = [[SunInfoView alloc] init];//日出日落
    [self.tableView addSubview:self.sunInfoView];
    
    if (iPhone4_4s) {
        
        self.weatherIconView.frame=CGRectMake(0, Height - W*2-50 ,W, W);//实时天气
        self.temperatureView.frame=CGRectMake(W, Height - W*2-50, W, W);//实时温度
        self.humidityView.frame =CGRectMake(0, Height - W-50, W, W);//湿度
        self.sunInfoView.frame =CGRectMake(W, Height - W-50, W, W);//日出日落
        ...
        self.maxTempView = [[MaxTempView alloc] init];//最高最低温
        self.windSpeedView = [[WindSpeedView alloc] init];//风向
        [self.tableView addSubview:self.maxTempView];
        [self.tableView addSubview:self.windSpeedView];
        
        self.weatherIconView.frame=CGRectMake(0, Height - W*3 ,W, W);//实时天气
        self.temperatureView.frame=CGRectMake(W, Height - W*3, W, W);//实时温度
        self.humidityView.frame =CGRectMake(0, Height - W*2, W, W);//湿度
        self.sunInfoView.frame =CGRectMake(W, Height - W*2, W, W);//日出日落
        self.maxTempView.frame =CGRectMake(0, Height - W, W, W);//最高最低温
        self.windSpeedView.frame =CGRectMake(W, Height - W, W, W);//风向
    ...
    // 创建出线条
    self.grayLine1                 = [[LeftToRightView alloc] initWithFrame:CGRectMake(0, self.weatherIconView.y, Width, 0.5)];
    self.grayLine1.backgroundColor = [UIColor blackColor];
    self.grayLine1.alpha           = 0.1;
    [self.tableView addSubview:self.grayLine1];
    
    self.grayLine2                 = [[LeftToRightView alloc] initWithFrame:CGRectMake(0, self.humidityView.y, Width, 0.5)];
    self.grayLine2.backgroundColor = [UIColor blackColor];
    self.grayLine2.alpha           = 0.1;
    [self.tableView addSubview:self.grayLine2];
    
    self.grayLine3                 = [[LeftToRightView alloc] initWithFrame:CGRectMake(0, self.weatherIconView.y + Width, Width, 0.5)];
    self.grayLine3.backgroundColor = [UIColor blackColor];
    self.grayLine3.alpha           = 0.1;
    [self.tableView addSubview:self.grayLine3];
    
    self.vLine                     = [[UpToDownView alloc] initWithFrame:CGRectMake(Width / 2.f - 1, self.weatherIconView.y, 0.5, Width)];
    ...
    if (iPhone4_4s) {
        // 创建出线条
        self.vLine.frame = CGRectMake(Width / 2.f - 1, self.weatherIconView.y, 0.5, Width);
    }else{
        ...
        self.vLine.frame = CGRectMake(Width / 2.f - 1, Height - Width - Width / 2.f, 0.5, Width + Width / 2.f);
    }
    ...
    // tableView上面加载的view
    self.shapeWordView           = [[ShapeWordView alloc] initWithFrame:CGRectMake(0, -40, Width, 20)];
    self.shapeWordView.text      = @"Release To Refresh";
    self.shapeWordView.font      = [UIFont fontWithName:LATO_THIN size:20.f];
    self.shapeWordView.lineWidth = 1.0f;
    self.shapeWordView.lineColor = [UIColor colorWithRed:0 green:128/255.0 blue:128/255.0 alpha:0.7];
